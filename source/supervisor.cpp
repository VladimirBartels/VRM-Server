...
void Supervisor::createCase1()
{
    ...
    // create cars
    Car *car1 = new Car(0, 0, 80, 100, Qt::darkBlue, id_1, eVSlow, _border);
    Car *car2 = new Car(0, 0, 80, 100, Qt::darkRed,  id_2, eVSlow, _border);
...
void Supervisor::createCase2()
{
    ...
    // create cars
    // !!! for now it does not matter the position, because after adding to scene, each item position will be (0, 0)
    Car *car1 = new Car(0, 0, 80, 100, Qt::darkYellow, id_1, eVNormal, _border);
    _garage.push_back(car1);
...
void Supervisor::createCase3()
{
    ...
    // create cars
    // !!! for now it does not matter the position, because after adding to scene, each item position will be (0, 0)
    Car *car1 = new Car(0, 0, 40, 50, Qt::darkMagenta, id_1, eVSlow, _border);
    Car *car2 = new Car(0, 0, 40, 50, Qt::darkGreen,   id_2, eVSlow, _border);
    Car *car3 = new Car(0, 0, 40, 50, Qt::darkBlue,    id_3, eVSlow, _border);
    Car *car4 = new Car(0, 0, 40, 50, Qt::darkYellow,  id_4, eVSlow, _border);
...
void Supervisor::createCase4()
{
    init(); // set initial state

    _activeCase = 4;

    // start tcp server to listen to clients
    _server->doConnect();
...
void Supervisor::startStopCase()
{
    ...
    if (_activeCase == 1 || _activeCase == 4)
    {        
        if (_isCaseStarted == false)
        {
            foreach (Car *car, _garage)
            {
                ...
                // TODO: temporary
                _server->sendChangeSpeed(car->getId(), 0);

                // move a real car
                if (car->direction() == eForward)
                {
                    _server->sendMoveForward(car->getId());
                }
                else if (car->direction() == eBackward)
                {
                    _server->sendMoveBackward(car->getId());
                }
            }
...
void Supervisor::changeSpeed(qint8 speed)
{
    ...
    foreach (Car *car, _garage)
    {
        // cahnge speed on a VRM car
        car->setSpeed(_speed);

        // change speed on a client car
        _server->sendChangeSpeed(car->getId(), speed);
    }
...
void Supervisor::sceneChanged(const QList<QRectF> &)
{
    ...
    foreach (Car *car, _garage)
    {
        ...
        // <<< ONLY FOR CASE 1 >>>
        if ((_activeCase == 1 || _activeCase == 4) && _isCaseStarted)
        {
